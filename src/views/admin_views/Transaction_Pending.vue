  ...
  <div class="bg-[#020617] min-h-screen flex relative">
    ...
    <div class="flex-1 ml-[240px]">
      ...
      <main class="pt-[90px] p-8">
        <h1 class="text-3xl text-white font-bold mb-6">Transações Pendentes</h1>
            ...
            <tbody>
              ...
              >
                ...
                <td class="px-6 py-4 flex justify-center gap-2">
                  <button
                    class="px-3 py-1 bg-green-600 hover:bg-green-700 rounded text-sm transition disabled:opacity-50"
                    :disabled="loadingId === tx.id"
                    @click="updateTransactionStatus(tx.id, 'Pago')"
                  >
                    {{ loadingId === tx.id ? 'Processando...' : 'Pago' }}
                  </button>
                  <button
                    class="px-3 py-1 bg-red-600 hover:bg-red-700 rounded text-sm transition disabled:opacity-50"
                    :disabled="loadingId === tx.id"
                    @click="updateTransactionStatus(tx.id, 'Falhado')"
                  >
                    {{ loadingId === tx.id ? 'Processando...' : 'Cancelar' }}
                  </button>
                </td>
              </tr>
